...
with torch.amp.autocast(device_type="cuda" if torch.cuda.is_available() else "cpu", dtype=torch.float32):
    # get prequantized representation
    encoded, encoded_len = codec_model.encode_audio(audio=input_audio, audio_len=input_audio_len)
    ...
    dequantized = codec_model.dequantize(tokens=gen_tokens, tokens_len=gen_tokens_lens)
    dequantized = dequantized.transpose(1, 2) # make it [B, T, F]